...
private ["_MaxIntensity","_maxspeed","_Boost","_leftengine","_rightengine","_emitters","_Intensity","_looptime", "_plane"];
		...
		while {(alive _plane)} do
		{
			...
			_leftengine setParticleParams ["\SU_33_Flanker_D\cl_exp","","Billboard",1,0.07,[0,-6.5,-0.9],[(velocity _plane select 0) - ((vectorDir _plane) select 0)*20,(velocity _plane select 1) - ((vectorDir _plane) select 1)*20,(velocity _plane select 2) - ((vectorDir _plane) select 2)*20],1,1.2745,1,0,[0.4+(2*(speed _plane/_maxspeed)),0.5+(10*(speed _plane/_maxspeed))],[[0.040,0.100,0.900,0.1500*_Intensity],[0.200,0.200,0.800,0.1000*_Intensity],[0.500,0.200,0.000,0.0250*_Intensity],[0.000,0.000,0.000,0.0000*_Intensity]],[0],0,0,"","",(_plane selectionPosition "EngineL")];
			_rightengine setParticleParams ["\SU_33_Flanker_D\cl_exp","","Billboard",1,0.07,[0,-6.5,-0.9],[(velocity _plane select 0) - ((vectorDir _plane) select 0)*20,(velocity _plane select 1) - ((vectorDir _plane) select 1)*20,(velocity _plane select 2) - ((vectorDir _plane) select 2)*20],1,1.2745,1,0,[0.4+(2*(speed _plane/_maxspeed)),0.5+(10*(speed _plane/_maxspeed))],[[0.040,0.100,0.900,0.1500*_Intensity],[0.200,0.200,0.800,0.1000*_Intensity],[0.500,0.200,0.000,0.0250*_Intensity],[0.000,0.000,0.000,0.0000*_Intensity]],[0],0,0,"","",(_plane selectionPosition "EngineR")];
